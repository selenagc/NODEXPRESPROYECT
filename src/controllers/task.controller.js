...
export async function store(req, res) {
  try {
    const { title, description, category_id, user_id, tag_ids = [] } = req.body;
    ...
    res.status(201).json({
      task: taskDecorator(taskWithRelations)
    });
...
export async function show(req, res) {
  try {
    ...
    res.json(taskDecorator(taskWithRelations));
...
export async function update(req, res) {
  try {
    const { id } = req.params;
    const { title, description, status, category_id, user_id, tag_ids } = req.body;
